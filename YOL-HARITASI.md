...
| **★ `APARTMAN_YONETICISI` deftere yazamıyor** | **P1** | Rol `FINANS_TAHAKKUK` · `FINANS_TAHSILAT` · `FINANS_DEFTER_GORUNTULE` taşıyor ama **`FINANS_YEVMIYE_GIRIS` taşımıyor** (`roller.ts:36-50`). Sonuç: tahakkuk çalıştırabiliyor ama onu **deftere geçiremiyor**; virman da yapamıyor. Kendi sitesini yöneten bir apartman yöneticisi `CIFT_TARAFLI` muhasebe kullanıyorsa yevmiye yolunun tamamı ona kapalı. ⚠️ CT-19 bu yüzden `YONETIM_SIRKETI` rolüyle koşuyor — **rol tanımı gevşetilmedi**, karar bekliyor. Alt soru: taşınma virmanı deftere hiç yazmıyorsa ondan da yevmiye izni istemek doğru mu? (İzin guard aşamasında, gövdeye bakılmadan kontrol edilir.) |
