...
| `[ORTAK]` **Birim testleri vitest süitine taşınsın mı** | P2 | Bugün **iki koşucu** var: birim testleri `node:test` + `.mjs` + `dist/` deseni (**352**), sözleşme testleri `vitest` + `.spec.ts` (**189**). İkisi de CI'da **ayrı adım** olarak koşuyor — yani bağlantı **var**, sorun kaçırılma değil. ★ Taşıma bir **İYİLEŞTİRMEDİR, düzeltme değil**: tek koşucu tek rapor demek. ⚠️ Ama `.mjs` + `dist/` deseni **bilinçli bir tercihti** — kök `tsconfig.json` `files: []` taşıyan bir **çözüm dosyasıdır** ve `.test.ts` dosyaları `projectService` tarafından hiçbir projede bulunamadığı için **ESLint düşüyor** ([sekme-hata.test.mjs](tests/unit/sekme-hata.test.mjs) başlığında yazılı). ★ Taşınacaksa **ESLint/tsconfig deseni birlikte çözülmeli**; R1 gereği yalnızca *"tek koşucu daha temiz"* sebep sayılmaz. ✅ Yerel tek kapı **kuruldu** (`pnpm test`), taşıma olmadan da tek komut ikisini koşuyor. |
...
| `[ORTAK]` **★ Hata metinlerinde KARMA TÜRKÇE — iki katman iki yazım** | **P2** | `shared/*-domain` katmanının hata metinleri **diakritiksiz** (`hisse oranlari toplami 1 degil`, `Tapu ve hisse kayitlarini duzeltin`), `backend/` katmanının metinleri **diakritikli** (`tahakkuk deftere geçirilmez`). İki katman ayrı ayrı tutarlı olduğu için bugüne kadar görünmedi. ⚠️ **8 Ağustos 2026'da Σ hisse kapısı bağlanınca görünür oldu:** bağlama katmanı domain mesajını sarıp üstüne kendi metnini eklediği için ikisi TEK CÜMLEDE birleşti ve kullanıcıya karma metin çıktı — `2 nolu bağımsız bölüm — … hisse oranlari toplami 1 degil (2.000000). … Kayıtlı hisseler: 1/1 (dönem içi)`. ★ **Yerel yama YAPILMADI:** bu depo genelinde tek karar gerektirir (domain diakritiğe geçsin mi, yoksa ASCII bilinçli bir taşınabilirlik tercihi mi) ve `shared/` altındaki onlarca dosyayı kapsar. Tek fonksiyonun metnini düzeltmek dikişi kapatmaz, domain içine ikinci bir tutarsızlık ekler. Mevcut hâl [hisse-kapisi.spec.ts](backend/test/contract/hisse-kapisi.spec.ts) `(2)`'de **sabitlendi** — tercih değişirse test kırmızıya düşer ve karar bilinçli alınır. |
