...
| **★ Mock varsayılanı ölçülmedi** | **P1** | `MOCK_AKTIF = (NEXT_PUBLIC_MOCK ?? '1') !== '0'` — varsayılan AÇIK ve anahtar `.env`'de hiç geçmiyor. `muhasebe`/`makbuzlar` blokları mock kullanmıyor, ötekiler kullanıyor. ★ **Varsayılan `'0'` yapılınca hangi ekranlar kırılır — ölçülmedi.** Yazılacak her yeni ekranı etkiler; muhasebe sekmesi tamamlanmadan önce cevaplanmalı. Ayrıntı: [REFERANS-ENVANTER §3](docs/REFERANS-ENVANTER.md). |
| **`/belgeler` menüde var, sayfa yok** | P2 | Menüden tıklanınca **404**. Backend'de 12 belge ucu çalışıyor. Sayfa mı yazılacak, menüden mi kaldırılacak — karar bekliyor. |
