...
| **Mock varsayılanı `'1'`** | P2 | ✅ **ÖLÇÜLDÜ (3 Ağustos):** `NEXT_PUBLIC_MOCK=0` ile **hiçbir ekran kırılmıyor** — 19 ekranın çağırdığı uçların tamamı 200 döndü. Sebep: `MOCK_AKTIF` yalnızca 7 yerde ve altısı genel yardımcıların içinde; **mock-only fonksiyon yok**. ⚠️ Ölçümün sınırı: uçların YANIT VERDİĞİ kanıtlandı, yanıt ŞEKLİNİN ekranın beklediğiyle aynı olduğu **kanıtlanmadı** — frontend tipleri `Mock*` adını taşıyor, yani mock'tan yazılmışlar. İki tip sınandı (`MockYerlesimOzeti` · `MockDaireKarti`), ikisi de uyuyor; kalanlar sınanmadı. **Kalan iş:** varsayılanın `'0'` yapılması ve mock açıkken ekranda uyarı bandı — karar bekliyor. [REFERANS-ENVANTER §3](docs/REFERANS-ENVANTER.md) |
| **Sınanmamış tipler — `Mock` öneki kasten duruyor** | P2 | CT-22, **doğrulanmış 19 tipten** öneki kaldırdı (11 yanıt + 7 iç içe + `AuditSatiri`). **15 tip sınanmadı ve öneki KORUNDU** — ad şu an doğru bilgi veriyor. İkiye ayrılır: **(a) İstek tipleri (9)** — `KisiGirdisi` · `PlakaGirdisi` · `MalikEkle` · `KiraciEkle` · `SakinEkle` · `GiderTuruGirdisi` · `PersonelGirdisi` · `DaireGorevlisiGirdisi` · `MisafirGirdisi`. Bunlar API'nin **kabul ettiği** şekildir, ayrı bir sözleşmedir. **(b) Kapsanmamış yanıt tipleri (6)** — `PortfoyOzeti` · `PortfoyProjesi` · `Sertifika` · `Zimmet` · `GorevliAraci` · `SakinCikisSonucu`; CT-22'nin uç listesinde yoklar. ⚠️ Risk aynı sınıf: eksik alan derleme zamanında görünmez — istekte 400, yanıtta boş hücre olarak çalışma anında çıkar. Sınandıklarında önek kaldırılır. |
