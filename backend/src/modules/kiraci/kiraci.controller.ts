...
import { KiraciCommandService, type TahliyeSonucu } from './kiraci.command.service';
...
export class KiraciController {
  ...
  @ApiOperation({
    ...
    description:
      ...
      'Bu kiracıya DAYANAN sakinlere (eşi · çocuğu · …) aynı işlemde OTOMATİK ' +
      'ÇIKIŞ verilir; çıkış tarihi tahliye tarihidir. Kaç kişinin çıkarıldığı ' +
      '`sakinCikisi.cikarilan` alanında döner; girişi tahliye tarihinden sonra ' +
      'olduğu için çıkarılamayan kayıtlar `sakinCikisi.cikarilamayan` içinde ' +
      'gerekçesiyle listelenir ve ELLE kapatılması gerekir.',
  })
  tahliyeEt(
    @Param('bolumId') bolumId: string,
    @Param('kiraciId') kiraciId: string,
